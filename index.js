...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(array){
    const rayOfStrings = [];
  array.forEach((item) => {
    rayOfStrings.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`)
  })
    return rayOfStrings;
  }
  ...
  */
  ...
  function lowerCaseNames(array){
    const newArr = array.map((item) => {
      const lowerName = item.animal_name.toLowerCase();
      return `${lowerName}`
  ...
  */
  ...
  function lowPopulationAnimals(array){
    const filtered = array.filter(x => x.population <5)
    return filtered;
  }
  ...
  */
  ...
  function USApop(array){
    const newArr = array.reduce((acc, element) => acc + element.population,0);  
    return newArr;
  }
